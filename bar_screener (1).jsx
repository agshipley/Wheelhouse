...
export default function App() {
      ...
      try {
        if (typeof window.storage !== "undefined") {
          for (const md of ["boardwalk", "custom"]) {
            try {
              const o = await window.storage.get(`wh:opps:${md}`);
              if (o && o.value) setOppsByMode((s) => ({ ...s, [md]: JSON.parse(o.value).map(normalizeOpp) }));
            } catch (e) {}
            try {
              const pr = await window.storage.get(`wh:params:${md}`);
  ...
  /* persist */
  useEffect(() => {
    if (!loaded || typeof window.storage === "undefined") return;
    (async () => { try { await window.storage.set(`wh:opps:${mode}`, JSON.stringify(opps)); } catch (e) {} })();
  }, [opps, mode, loaded]);
  useEffect(() => {
    if (!loaded || typeof window.storage === "undefined") return;
    (async () => { try { await window.storage.set(`wh:params:${mode}`, JSON.stringify(params)); } catch (e) {} })();
  }, [params, mode, loaded]);
